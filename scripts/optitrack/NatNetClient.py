...
class NatNetClient:
    ...
    def __create_data_socket( self, port ):
        ...
        if self.use_multicast:
            ...
            result.bind( (self.local_ip_address, port) )
            # try:
            #     # Bind to the specific data port (1511) so Motive's unicast frames
            #     # arrive here.  Binding to port 0 (ephemeral) is wrong: Motive sends
            #     # data to the client at the negotiated data_port, not a random port.
            #     result.bind( ('', port) )
            # except socket.error as msg:
            #     print("ERROR: data socket error occurred:\n%s" %msg)
            #     print("Check Motive/Server mode requested mode agreement.  You requested Unicast ")
            ...
            #     print("ERROR: data socket timeout occurred. Server not responding")
            #     result = None
            # Do NOT join a multicast group in unicast mode — joining 239.255.42.99
            # here is wrong and throws an unhandled exception on Windows.
        ...
        if( ( major == 2 and minor > 3 ) or major > 2 ):
            ...
            for _ in range( 0, labeled_marker_count ):
                ...
                labeled_marker_from_model_id[model_id][marker_id] = {
                    ...
                    'error' : residual,
                    'param' : param,
                    'is_valid' : (param & 0x01) == 0,  # bit0=occluded
                }
