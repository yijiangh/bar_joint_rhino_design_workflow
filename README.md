...
- `T1`: line-distance optimization for the new bar axis
- `T2`: joint-placement optimization for the connector blocks on the solved bars
...
The Rhino buttons in this repo call only the Rhino entry-point scripts:

- `scripts/bake_frame.py`
- `scripts/closest_line_segment.py`
- `scripts/export_cad_config.py`
- `scripts/t1_bar_axis.py`
- `scripts/t1_bar_axis_rerun.py`
- `scripts/t2_joint_placement.py`
- `scripts/t2_joint_placement_rerun.py`

Those scripts do not require PyBullet. They only need the core runtime from `requirements.txt`.

Use Rhino 8's own CPython interpreter so the `_-ScriptEditor _R` commands can import the installed packages.

Do not rely on `sys.executable` for this lookup. In Rhino 8 it can report the host app path such as `C:\Program Files\Rhino 8\System\Rhino.exe` instead of the deployed CPython runtime.

Rhino 8 deploys its CPython runtime under the RhinoCode root directory:

- Default Windows root: `%USERPROFILE%\.rhinocode`
- Default macOS root: `~/.rhinocode`
- Rhino 8 CPython runtime folder: `py39-rh8`

If `py39-rh8` does not exist yet, run the `ScriptEditor` command once and let Rhino finish initializing Python.

If your Rhino install uses a custom RhinoCode root, replace `.rhinocode` below with the value of `RhinoCodePlugin.RootPath` from Rhino `Tools -> Options -> Advanced`.

Windows default path:

```powershell
& "$HOME\.rhinocode\py39-rh8\python.exe" -m pip install -r requirements.txt
```

Windows example from this project:

```powershell
& "C:\Users\yijiangh\.rhinocode\py39-rh8\python.exe" -m pip install -r requirements.txt
```

macOS default root:

```bash
RHINO_PY="$(find "$HOME/.rhinocode/py39-rh8/bin" -type f \( -name python -o -name 'python3*' \) | head -n 1)"
"$RHINO_PY" -m pip install -r requirements.txt
```

If Rhino still cannot import `numpy` or `scipy`, you almost certainly installed into a different Python than the `py39-rh8` runtime that Rhino ScriptEditor is using.
...
Recommended order to kickstart Rhino use:

1. Install the core runtime into Rhino 8 CPython.
2. Use `Bake Frame` to create the named CAD reference frames in Rhino.
3. Run `Export CAD Config` after those reference frames are updated.

The above only need to be re-done if joint definition changes.

4. Use `T1 Bar` to run the line-distance optimization and place or reposition the new bar axis.
5. Use `T2 Joint` to run the joint-placement optimization, preview the connector assembly variants, and place the connector block instances.

Recommended test file:

- Start from `HalfJointV7_template.3dm` when testing the Rhino 8 workflow.
- Its main value is that it already contains the frame annotations expected by `scripts/export_cad_config.py`.
- It also contains ready-made `FemaleLinkBlock` and `MaleLinkBlock` definitions authored at the world origin in the OCF convention expected by the joint-placement workflow, so the T2 buttons work out of the box.
- If you move this workflow into another Rhino file, you only need to bring over block definitions with the same block names for T2 joint placement to work.
- Re-run `scripts/export_cad_config.py` whenever the joint definition changes, or when adapting the workflow to a new joint family such as subfloor or ground joints.

### Create Rhino buttons manually

`scripts/generate_rhino_toolbar.py` exists, but the automatic `.rui` generation is not the recommended install path yet. For now, create Rhino buttons manually and paste the script macros directly.

General notes:

- Create or open a custom toolbar in Rhino.
- Add one new button per script, or pair the rerun scripts onto right-click if you prefer.
- Paste the macro into the button command field and give the button a readable label.
- Use an absolute script path.
- Forward slashes work on both Windows and macOS.
- Use `_-ScriptEditor _R`, not `_-RunPythonScript`.
- Keep the Rhino buttons limited to the Rhino entry-point scripts listed above. Do not wire the PyBullet scripts into Rhino buttons.

Macro template:

```text
! _-ScriptEditor _R "/absolute/path/to/bar_joint_rhino_design_workflow/scripts/<script_name>.py"
```
