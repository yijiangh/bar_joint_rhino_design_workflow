...
Those scripts do not require PyBullet. They only need `numpy` and `scipy`.

**No manual pip install is needed.** Each Rhino entry-point script declares its own virtual environment and requirements using Rhino 8 ScriptEditor directives:
...
1. Use `Bake Frame` to create the named CAD reference frames in Rhino.
2. Run `Export CAD Config` after those reference frames are updated.

The above only need to be re-done if joint definition changes.

3. Use `T1 Bar` to run the line-distance optimization and place or reposition the new bar axis.
4. Use `T2 Joint` to run the joint-placement optimization, preview the connector assembly variants, and place the connector block instances.
